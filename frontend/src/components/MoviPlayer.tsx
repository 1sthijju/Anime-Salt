...
export function MoviPlayer({
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      el.addControl({ id: 'skip-intro', label: 'Skip Intro',
        icon: '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M4 18l8.5-6L4 6v12zm9-12v12l8.5-6L13 6z"/></svg>',
        placement: 'both', hotkey: 'shift+i',
        onSelect: () => { if (onSkipIntro) onSkipIntro(); else if (el.seek) el.seek(90); },
      });
      el.addControl({ id: 'skip-outro', label: 'Skip Outro',
        icon: '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M6 6h2v12H6zm3.5 6l8.5 6V6z"/></svg>',
        placement: 'both', hotkey: 'shift+o',
        onSelect: () => { if (onSkipOutro) onSkipOutro(); else if (el.seek && el.duration) el.seek(Math.max(0, el.duration - 90)); },
      });
      if (nextEpisodeSlug || onNextEpisode) {
        el.addControl({ id: 'next-episode', label: 'Next Episode',
          icon: '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M6 18l8.5-6L6 6v12zM16 6v12h2V6h-2z"/></svg>',
          placement: 'both', hotkey: 'shift+n',
          onSelect: () => { if (onNextEpisode) onNextEpisode(); },
        });
      }
      el.addControl({ id: 'ambient-mode', label: 'Ambient Mode',
        ...
        placement: 'both', toggle: true, active: ambientMode, hotkey: 'g',
        onSelect: (on: boolean) => { setAmbientMode(on); el.setAttribute('ambient', on.toString()); },
      });
    }, 500);
  ...
  useEffect(() => {
    ...
    const readTracks = () => {
      ...
      onAudioTracks(Array.from(list).map((t: any, i: number) => ({
